...
|---|---|---|---|
| DES | 是 | 否 | DES 的某些 round components 本身不是 invertible，但由於整體結構的安排，整個 round 仍然可以被反向還原。 |
| Rijndael | 是 | 是 | Rijndael 不僅整個 round 是 invertible，其 round 中的主要 transformations 也都是 invertible。 |
...
block cipher 的安全性並不是自動成立的。密碼分析者可以透過不同方法來嘗試恢復 key，或找出演算法的弱點。從廣義上看，對 block cipher 的分析既包括直接的搜尋與分解技巧，也包括特定的攻擊模型與統計分析方法，例如 exhaustive search、pre-computed tables、divide-and-conquer、chosen plaintext attack、differential cryptanalysis 與 linear cryptanalysis。
...
<div class="remark">
  <p>Differential cryptanalysis 與 linear cryptanalysis 都是非常成功且影響深遠的密碼分析技術。它們對 block cipher 設計產生了直接影響，也成為評估新 cipher 時不可忽略的標準。</p>
...
因此，modern block cipher 的安全性是**多輪結構與整體設計**共同作用的結果。這也是為什麼現代 block cipher 常被理解為多輪 substitution-permutation 結構，或更廣義地說，是透過反覆混淆與擴散來建立安全性。
...
block cipher 是一種對固定長度 block 進行可逆轉換的 symmetric primitive。其安全性通常來自多輪 iterated structure、round function、round key 與 key schedule 的配合設計。DES 是歷史上最重要的早期標準，而 AES（Rijndael）則是現代最具代表性的 block cipher 標準。對 block cipher 的理解不僅涉及其加密結構，也涉及 modes of operation、invertibility、substitution and permutation，以及對 differential cryptanalysis 與 linear cryptanalysis 等攻擊方法的抵抗能力。
