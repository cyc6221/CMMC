...
其中最具代表性的早期標準是 DES，而現代最重要的標準則是 AES。
...
最後，NIST 於 2000 年宣布 **Rijndael** 為最終勝出者。當 Rijndael 被標準化後，即成為今日所稱的 **AES**。
...
為了使 decryption 成立，整個 round transformation 必須是 invertible。然而，這**不代表 round 內部每一個子函數都必須 individually invertible**。

這一點在 DES 與 Rijndael 的設計差異上尤其明顯。

### DES

在 DES 中，用來構成 round 的部分函數並不是 invertible 的；但是由於整體 round 結構的安排，整體加密過程仍然可以被正確解密。

### Rijndael

在 Rijndael 中，不僅整個 round 是 invertible 的，構成 round 的每一個主要 transformation 也都是 invertible 的。這反映了 Rijndael 與 DES 在設計結構上的不同。

## General Approaches to Breaking a Block Cipher

block cipher 的安全性並不是自動成立的。密碼分析者可以透過不同方法來嘗試恢復 key，或找出演算法的弱點。一般性的攻擊方式包括：

- exhaustive search
- pre-computed tables of intermediate values
...
某些攻擊可能預先計算大量中間值並存成表格，以在攻擊階段加速搜尋。這類方法通常涉及時間與空間的 trade-off。

### Divide and Conquer

某些密碼結構允許攻擊者將一個大問題拆成多個較小的問題分別處理，再將結果整合，從而降低整體攻擊成本。
...
**Differential cryptanalysis** 是分析 block cipher 的經典方法之一。

其基本想法是觀察兩個 plaintexts 之間的某種特定差異，並研究這種差異在加密過程中如何傳播到 ciphertext。常見的差異度量是 XOR difference，因此分析者會研究：

- plaintext pair 的 XOR difference
- ciphertext pair 的 XOR difference

若某些差異在通過多輪加密後，出現特定輸出差異的機率顯著偏離隨機情況，則這些統計偏差可能洩漏 key 相關資訊。

因此，differential cryptanalysis 可以視為一種基於**差異傳播與機率偏差**的統計分析方法。

## Linear Cryptanalysis

**Linear cryptanalysis** 也是 block cipher 分析中的重要技術。

好的 block cipher 應包含 non-linear components，但 linear cryptanalysis 的想法是：嘗試用某些 linear expressions 去近似這些 non-linear components 的行為。如果某些線性關係出現的機率與真正隨機情況相比存在可觀察偏差，則這種偏差可用來推測 key 的部分資訊。

因此，linear cryptanalysis 本質上是一種**利用線性近似與統計偏差**來攻擊 cipher 的方法。

## Success of Classical Cryptanalysis

Differential cryptanalysis 與 linear cryptanalysis 都是非常成功且影響深遠的密碼分析技術。它們對 block cipher 設計產生了直接影響，也成為評估新 cipher 時不可忽略的標準。

不過，這並不表示所有重要 cipher 都會被它們實際攻破。對於設計良好的 block ciphers，例如 DES 與 Rijndael，這些方法雖然是分析時必須考慮的經典攻擊，但並未使其在原設計參數下失去實用安全性。
