...
由於 block cipher 一次只能處理固定大小的一個 block，而實際訊息通常長於單一 block，因此需要一套方法將多個 block 串接起來加密，這就是 **[Modes of Operation]({{ "/articles/CryptoAnIntro/modes-of-operation/" | relative_url }})**。
...
<div class="example">
  ...
  常見的 block cipher modes of operation 包括：
  <ul>
...
**[DES]({{ "/articles/CryptoAnIntro/des/" | relative_url }})**（Data Encryption Standard）是最著名的早期 block cipher 之一。它在 1970 年代中期被採用為美國聯邦標準，之後也迅速成為銀行應用中的事實標準。
...
DES 在其時代是一個極具影響力的設計，但到了 1990 年代初，逐漸顯露出不足之處。主要原因包括：
...
最後，NIST 於 2000 年宣布 **[Rijndael]({{ "/articles/CryptoAnIntro/rijndael/" | relative_url }})** 為最終勝出者，並在其標準化後成為今日所稱的 **AES**。
...
這兩類操作在古典密碼中就已出現，但現代 block cipher 將它們設計得更加精細與複雜。單獨使用 substitution 或 permutation 並不自動產生安全性。真正的安全性來自於：
