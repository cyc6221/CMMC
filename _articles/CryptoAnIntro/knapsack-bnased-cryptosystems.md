...
<ul>
  <li>$70 \ge 52$，所以選 $52$，剩下 $18$；</li>
  <li>$18 < 27$，所以不選 $27$；</li>
  <li>$18 \ge 13$，所以選 $13$，剩下 $5$；</li>
  <li>$5 < 6$，所以不選 $6$；</li>
  <li>$5 \ge 3$，所以選 $3$，剩下 $2$；</li>
  <li>$2 \ge 2$，所以選 $2$，剩下 $0$。</li>
</ul>
...
<div class="remark">
<strong>Remark.</strong>
這裡的 super-increasing sequence 本身不能直接公開，因為它太容易被解。Merkle--Hellman 的核心構想，就是用模乘把這個 easy instance 包裝成一個外觀看似隨機的 hard instance。
</div>
...
<div class="remark">
<strong>Remark.</strong>
從加密者的角度來看，這只是單純解一個公開的加權總和；真正讓系統成立的是，合法接收者可以利用私鑰把這個總和重新拉回 super-increasing knapsack。
</div>
...
LLL algorithm 是 Lenstra--Lenstra--Lovász 提出的 lattice basis reduction 演算法。它的目的是把一組 lattice basis 轉換成另一組較短且較接近正交的 basis。雖然 LLL 不一定能找出最短向量，但在很多實務情況下，它能有效找到相當短的向量，因此成為分析格問題的重要工具。
...
<div class="remark">
<strong>Remark.</strong>
這裡的關鍵觀察是：原本的 bit solution 會對應到 lattice 中的一個特別短的向量。於是攻擊者不必直接枚舉所有 bit string，而是改成尋找短向量。
</div>
