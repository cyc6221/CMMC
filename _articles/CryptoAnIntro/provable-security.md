...
> If an adversary is able to break a certain notion of the security of a system then one could use the adversary to do something believed to be impossible.
...
- 若 adversary $A$ 能有效攻擊某個 scheme；
- 則可利用 $A$ 構造出另一個 algorithm $B^A$；
- 而 $B^A$ 可以進一步有效解決某個 hard problem。

這種證明方式在 **passive attack** 的模型下通常較容易處理，因為 adversary 只能觀察 public information 與 challenge ciphertext，不能額外查詢 decryption oracle、signature oracle 或其他互動式 oracle。  
因此，reduction 往往不必模擬複雜的互動環境，而只需利用 adversary 的 distinguisher 或 attack procedure，便可將其轉換成一個解決底層困難問題的 algorithm。

<div class="example">
典型例子包括：將 ElGamal 的 semantic security 歸約到 DDH 假設，以及將 Goldwasser--Micali 的安全性歸約到 QUADRES problem。
</div>
...
<strong>Random Oracle Model</strong>

A random oracle is an idealized hash function which on input of a new query will pick, uniformly at random, some response from its output domain, and which if asked the same query twice will always return the same response.
</div>
