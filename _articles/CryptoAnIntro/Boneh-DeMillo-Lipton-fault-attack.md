...
Boneh–DeMillo–Lipton Fault Attack 是針對 **CRT-based RSA implementation** 的經典 **fault attack** <a class="cite" href="#bib-bdl01">BDL01</a>。此攻擊指出：若 attacker 能在 RSA 簽章過程中故意使某一部分運算發生錯誤，則只要取得一個錯誤的 signature，就可能直接分解 RSA modulus $N$，進而 recover private key。

這類攻擊的重點不在於從數學上直接破解 RSA，而在於利用 **implementation fault**。也就是說，RSA 的數學結構本身可能沒有問題，但若裝置在實際計算時遭受外部干擾，例如 voltage glitch、clock glitch、加熱、冷卻或其他硬體層級的 fault injection，則輸出的錯誤結果可能洩漏出原本不應暴露的 secret information。
...
其中 $p,q$ 為兩個大質數，public exponent 為 $e$，private exponent 為 $d$。對於 message $m$，先計算其 hash
...
接著再使用 CRT 將兩個 residue 合併成最終 signature。課本中採用的寫法為
...
這表示錯誤 signature 在模 $q$ 的世界裡仍然看起來像合法 signature，但在模 $p$ 的世界裡已經失效。
...
## Why CRT Makes the Attack Dangerous

若 RSA signature 直接計算

$$
s = h^d \pmod N,
$$

則 fault 的影響通常較不容易以這種方式分離出單一質因數。CRT-based implementation 的問題在於它將一個整體運算拆成模 $p$ 與模 $q$ 兩條路徑。只要其中一條路徑 faulted、另一條路徑正常，輸出就會同時帶有「一邊正確、一邊錯誤」的結構，而這正是 gcd 攻擊能成功的原因。

因此，CRT 雖然能顯著提升 RSA implementation 的效率，但也使 fault attack 的威脅更加直接。

## Attack Requirements

Boneh–DeMillo–Lipton Fault Attack 的成立通常依賴以下條件：
...
在這些條件下，只需一次錯誤輸出，就可能直接 factor $N$。
...
此類防禦的核心思想是：即使內部某一步驟發生 fault，也不能讓錯誤結果直接暴露給 attacker。

## Remark

Boneh–DeMillo–Lipton Fault Attack 常被稱為 **Bellcore attack**。在多數文獻中，兩者通常指的就是這個針對 **CRT-RSA signature** 的經典 fault attack。
