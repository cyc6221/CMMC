...
- 一個 challenge $Y \in G$
- 一串 generators $X_1, X_2, \dots, X_n \in G^\*$
- 一個 oracle $\mathrm{DLO}$
...
也就是說，它至多只能對一個群元素求一次離散對數。（不過這個元素以及所選的基底 $X_i$，都可以由它自行決定。）

基底的個數 $n$ 是問題的一個參數，因此可以稱為 $n$-MBDL 問題或假設。（本文的結果實際上只會依賴 $1$-MBDL，但仍保留一般形式的定義，以供未來可能的應用。）要求至多只能呼叫一次 $\mathrm{DLO}$ 是必要的，因為如果允許兩次查詢，那麼

$$
\mathrm{DL}_{G,g}(Y)
=
\mathrm{DLO}(1,Y)\cdot \mathrm{DLO}(1,g)^{-1} \pmod p, \qquad where p = \lvert G \rvert
$$
