...
仍固定一個循環群 $G$ 與其生成元 $g$。在 MBDL 中，攻擊者會得到一個挑戰 $Y \in G$、一串生成元 $X_1, X_2, \dots, X_n \in G^\*$，以及一個 oracle $\mathrm{DLO}$。

此 oracle 在輸入 $(i, W)$ 時，回傳 $\mathrm{DL}_{G,X_i}(W)$，也就是 $W$ 對基底 $X_i$ 的離散對數，而不是對基底 $g$ 的離散對數。

攻擊者若要獲勝，必須輸出 $\mathrm{DL}_{G,g}(Y)$，也就是挑戰 $Y$ 對基底 $g$ 的離散對數，同時整體至多只能呼叫一次 $\mathrm{DLO}$。

也就是說，它至多只能對一個群元素求一次離散對數。（不過這個元素以及所選的基底 $X_i$，都可以由它自行決定。）基底的個數 $n$ 是問題的一個參數，因此可以稱為 $n$-MBDL 問題或假設。（本文的結果實際上只會依賴 $1$-MBDL，但仍保留一般形式的定義，以供未來可能的應用。）要求至多只能呼叫一次 $\mathrm{DLO}$ 是必要的，因為如果允許兩次查詢，那麼
