...
更一般地，對參數 $a$（在可交換級數與積分的收斂條件下，例如 $|a|\le 1$），有：
...
直覺上，這個結果之所以有名，是因為左邊的 integrand $x^{-x}=e^{-x\ln x}$ 不像能積出漂亮的值，而右邊 $\sum n^{-n}$ 也不是常見的特殊函數級數；但兩者竟然精確相等，因此被戲稱為「Sophomore’s dream」。
