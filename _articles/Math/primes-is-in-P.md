...
Fermat 小定理指出：若 $p$ 為質數且 $p\nmid a$，則 $a^{p-1}\equiv 1\pmod p$，因此可用快速冪有效率檢查 $a^{n-1}\equiv 1\pmod n$。此方法仍不足以構成正確的質數判定，因為合成數可能對某些 $a$ 通過，而 Carmichael 數甚至會對所有與 $n$ 互質的 $a$ 通過。[Car10]
...
後續 Lenstra 與 Pomerance 以 **Gaussian periods** 改寫 AKS 類方法，在**不依賴任何 Sophie Germain 相關假設**的情況下，也能達到約 $(\log n)^6\cdot(2+\log\log n)^c$ 的確定性時間界（其中 $c$ 為可有效計算的常數）。[LP05]
