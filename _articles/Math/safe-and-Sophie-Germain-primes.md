...
**Sophie Germain 質數猜想**主張：存在**無限多** Sophie Germain 質數 $q$。由於每個 Sophie Germain 質數 $q$ 都對應到一個 safe prime $p=2q+1$（反之亦然），因此此猜想等價於：存在**無限多** safe primes $p$ 使得 $p=2q+1$。目前此猜想**仍未被證明或否證**，屬於數論中的公開問題。
